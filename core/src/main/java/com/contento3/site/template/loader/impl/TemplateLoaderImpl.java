...
public class TemplateLoaderImpl implements TemplateLoader {
	...
	@Override
	public TemplateContentDto loadById(final Integer templateId){
...
//				}
				...
				if(resourceName.startsWith("/template_key") || resourceName.startsWith("template_key") ){
					dto = findDirectTemplateByKey(resourceName,siteId,"template_key/",false);
				}else{
					dto = findDirectTemplate(resourceName,siteId,"/template",false);
				}
			}
				...
				if(resourceName.startsWith("/global_template_key") || resourceName.startsWith("global_template_key") ){
					dto = findDirectTemplateByKey(resourceName,siteId,"global_template_key/",true);
				}else{
					dto = findDirectTemplate(resourceName,siteId,"/global_template",true);
				}
			}
	...
	private TemplateContentDto findDirectTemplate(String templatePath,Integer siteId,final String splitter,final Boolean isGlobal) throws Exception  {
		TemplateContentDto dto=null;
		final String[] templateName = templatePath.split(splitter,0);
		final SiteDto site = siteService.findSiteById(siteId);
		
		Integer accountId = site.getAccountDto().getAccountId();
		if (isGlobal){
			accountId = 1;
		}
		
		final TemplateDto template = templateService.findTemplateByPathAndAccount(templateName[1], accountId);
		dto = new TemplateContentDto();
	...
	private TemplateContentDto findDirectTemplateByKey(String templatePath,
			Integer siteId,final String splitter,final Boolean isGlobal) throws Exception {
		TemplateContentDto dto=null;
		...
		final TemplateDto template = templateService.findTemplateByKeyAndAccount(templateKey[1],accountId);
		dto = new TemplateContentDto();
		dto.setContent(template.getTemplateText());
		return dto;
	}
